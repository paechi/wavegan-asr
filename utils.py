...
def get_samples(train_files, window_length, overlap):
    train_samples = []
    for filepath in train_files:
        try:
            audio_data, _ = librosa.load(filepath, sr=16000)
            # Clip magnitude
        ...
        samples = []
        for i in range(0, 2 * audio_len, window_length):
            if i == 0:
                start = 0
                end = window_length
            else:
            ...
            if end >= audio_len:
                ...
                break
            sample = audio_data[start:end]
            samples.append(sample)
        train_samples.append(samples)
    train_samples = [sample for samples in train_samples for sample in samples]

    return train_samples

# Adapted from @jtcramer https://github.com/jtcramer/wavegan/blob/master/sample.py.
class WaveganDataset(Dataset):
    def __init__(self, data_path, window_length, generate=False):
        train_files = get_all_audio_filepaths(data_path)
        hr_train_files = [f for f in train_files if 'hr' in f]
        lr_train_files = [f for f in train_files if 'lr' in f]
        self.hr_data = get_samples(hr_train_files, window_length, overlap=2000)
        self.lr_data = get_samples(lr_train_files, window_length, overlap=2000)

    def __len__(self):
        return len(self.lr_data)

    def __getitem__(self, idx):
        hr_data = self.hr_data[idx]
        lr_data = self.lr_data[idx]
        return {'hr_data': hr_data,
                'lr_data': lr_data}

    def collate_fn(self, batch):
        hr_samples = [s['hr_data'] for s in batch]
        lr_samples = [s['lr_data'] for s in batch]
        return hr_samples, lr_samples


class GeneratorDataset(Dataset):
    def __init__(self, data_path, idx, window_length, generate=False):
        lr_train_file = get_all_audio_filepaths(data_path)[idx]
        print(lr_train_file)
        self.lr_data = get_samples([lr_train_file], window_length, overlap=2000)
    ...
    def __getitem__(self, idx):
        lr_data = self.lr_data[idx]
        return {'lr_data': lr_data}

    def collate_fn(self, batch):
        lr_samples = [s['lr_data'] for s in batch]
        return np.stack(lr_samples)
...
# def plot_loss(D_cost_train, D_wass_train, G_cost, save_path):
#     assert len(D_cost_train) == len(D_wass_train) == len(G_cost)
#
